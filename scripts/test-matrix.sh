...
deploy_targets=(cloudflare-workers vercel)
        ...
        for test in "${bools[@]}"; do
          for storage in "${bools[@]}"; do
            for storybook in "${bools[@]}"; do
              ...
              code=$?
              if [ $code -ne 0 ]; then
                # auth=cloudflare-access and enable_file_storage=true are
                # validator-rejected when deploy_target=vercel (see
                # copier.yml) -- expected, not a render bug. Recognize the
                # validator's own message text rather than skipping on
                # exit code alone, so a real render failure in this same
                # combination still counts as FAIL.
                if [[ "$out" == *"needs deploy_target=cloudflare-workers"* ]]; then
                  skipped=$((skipped+1))
                else
                  fail=$((fail+1))
                  failures+=("$name :: $out")
...
echo "TOTAL=$total SKIPPED(invalid combo)=$skipped RENDERED=$((total-skipped)) FAIL=$fail"
